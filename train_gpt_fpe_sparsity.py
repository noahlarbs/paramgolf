...
class Hyperparameters:
    ...
    d_ff_base = int(os.environ.get("D_FF_BASE", 256))
    d_ff_max = int(os.environ.get("D_FF_MAX", 1024))
...
def split_ffn_neurons(W1_T, W2_T, n_children=2):
    # W1_T: [d_ff, dim], W2_T: [dim, d_ff]
    device = W1_T.device
    d_ff, dim = W1_T.shape
    new_fc = []
    new_proj = []
    ...
    for j in range(d_ff):
        ...
        for c in range(n_children):
            noise = (torch.randn_like(w1_j) * 1e-5) if c > 0 else 0
            new_fc.append(w1_j + noise)
            new_proj.append(w2_j / n_children)
            
    return torch.cat(new_fc, dim=0), torch.cat(new_proj, dim=1)

class ExpandableMLP(nn.Module):
    # relu^2 MLP from the original modded-nanogpt setup with FPE growth
    ...
    def forward(self, x: Tensor, return_hidden=False):
        h = torch.relu(self.fc(x))
        self.latest_sparsity = (h.abs() < 1e-3).float().mean().detach().item()
        out = self.proj(h.square())
        if return_hidden:
            return out, h
        return out

    def expand_ffn(self, n_children=2):
        W1_new, W2_new = split_ffn_neurons(self.fc.weight.data, self.proj.weight.data, n_children=n_children)
        self.fc.weight = nn.Parameter(W1_new.to(dtype=self.fc.weight.dtype))
        self.fc.out_features, self.fc.in_features = W1_new.shape
        self.proj.weight = nn.Parameter(W2_new.to(dtype=self.proj.weight.dtype))
        self.proj.out_features, self.proj.in_features = W2_new.shape
        return W1_new.shape[0]
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = ExpandableMLP(dim, hidden)
        self.attn_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.mlp_scale = nn.Parameter(torch.ones(dim, dtype=torch.float32))
        self.resid_mix = nn.Parameter(torch.stack((torch.ones(dim), torch.zeros(dim))).float())

    def forward(self, x: Tensor, x0: Tensor, return_hidden=False):
        ...
        if return_hidden:
            mlp_out, h_states = self.mlp(self.mlp_norm(x), return_hidden=True)
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, return_hidden=False):
        ...
        for i in range(self.num_encoder_layers):
            if return_hidden and i == self.num_encoder_layers - 1:
                x, h_states = self.blocks[i](x, x0, return_hidden=True)
            else:
...
def main() -> None:
    ...
    def compile_model(m):
        cm = torch.compile(m, dynamic=False, fullgraph=True)
        return DDP(cm, device_ids=[local_rank], broadcast_buffers=False) if distributed else cm
        
    model = compile_model(base_model)

    def build_optimizers(m: GPT):
        block_named_params = list(m.blocks.named_parameters())
        matrix_params = [
            p
        ...
        if m.skip_weights.numel() > 0:
            scalar_params.append(m.skip_weights)
        token_lr = args.tied_embed_lr if args.tie_embeddings else args.embed_lr
        optimizer_tok = torch.optim.Adam(
            [{"params": [m.tok_emb.weight], "lr": token_lr, "base_lr": token_lr}],
        ...
        if m.lm_head is not None:
            optimizer_head = torch.optim.Adam(
                [{"params": [m.lm_head.weight], "lr": args.head_lr, "base_lr": args.head_lr}],
                betas=(args.beta1, args.beta2),
                eps=args.adam_eps,
                fused=True,
            )
    ...
    # FPE Loop Variables
    dpr_stats = []
    has_expanded_count = 0
    max_expansions = int(math.log2(args.d_ff_max / max(1, args.d_ff_base)))
    ...
    while True:
        ...
        if should_track_fpe:
            metric_val = sum(b.mlp.latest_sparsity for b in base_model.blocks) / len(base_model.blocks)
            
            # Smooth with a moving average
            if not dpr_stats:
                smoothed_pr = metric_val
            else:
                smoothed_pr = 0.3 * metric_val + 0.7 * dpr_stats[-1]
                
            dpr_stats.append(smoothed_pr)
            log0(f"[FPE] Step {step} | Raw Sparsity: {metric_val:.4f} | Smoothed: {smoothed_pr:.4f}")
            ...
            if len(dpr_stats) >= args.fpe_patience:
                ...
                if relative_growth < args.fpe_tolerance:
                    log0(f"\n>>> [FPE TRIGGER] Sparsity Plateaued (Relative Growth: {relative_growth*100:.2f}% < {args.fpe_tolerance*100:.2f}%). Expanding FFN...")
